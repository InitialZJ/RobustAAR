...
def run(weights=ROOT / 'best.pt',  # model.pt path(s)
    ...
    # flags
    long_gps_flag = True
    short_day_flag = False
    short_night_flag = True
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        pos_receiver = simulink_data[3:]
        pos_gps = np.array(pos_drogue) - np.array(pos_receiver)
        li_t.append(time.time() - t_start)
        li_x.append(pos_gps[0])
        li_y.append(pos_gps[1])
        li_z.append(pos_gps[2])
        print(len(li_t))
        if li_t[-1] > 55:
            csv_file = 'data.csv'
            data = zip(li_t, li_x, li_y, li_z)
            with open(csv_file, 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(['t', 'x', 'y', 'z'])  # 写入 CSV 文件的标题行
