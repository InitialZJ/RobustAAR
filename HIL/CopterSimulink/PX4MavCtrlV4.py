...
class EarthModel():
    ...
    def ecef2lla(self, x, y, z):
        ...
        ro = -(P * e2 * r) / (1 + Q) + math.sqrt(
            (self.wgs84_a * self.wgs84_a / 2) * (1 + 1 / Q) - (P * (1 - e2) * z2) / (Q * (1 + Q)) - P * r2 / 2)
        tmp = (r - e2 * ro) ** 2
        U = math.sqrt(tmp + z2)
...
class PX4MavCtrler:
    ...
    def __init__(self, port=20100, ip='127.0.0.1'):
        ...
        self.uavPosGPS = [0, 0, 0, 0, 0, 0, 0, 0,
                          0]  # Estimated GPS position from PX4 in NED frame,lat lon alt relative_alt vx vy vz hdg
    ...
    def sendStartMsg(self, copterID=-1):
        ...
        """
        self.udp_socket.sendto(buf,
                               ('224.0.0.10', 20007))  # multicast address '224.0.0.10' and port 20007 are adopted here
        ...
        while True:
            ...
            if self.EnList[1] != 0:  # if speed mode
                ...
                if self.coordinate_frame == mavlink2.MAV_FRAME_BODY_NED:
                    targetPosE[0] = self.pos[0] * math.cos(self.uavAngEular[2]) + self.pos[1] * math.sin(
                        self.uavAngEular[2])
                    targetPosE[1] = self.pos[0] * math.sin(self.uavAngEular[2]) + self.pos[1] * math.cos(
                        self.uavAngEular[2])
            ...
            bodyVx = self.uavVelNED[0] * math.cos(-self.uavAngEular[2]) + self.uavVelNED[1] * math.sin(
                -self.uavAngEular[2])
            bodyVy = self.uavVelNED[0] * math.sin(-self.uavAngEular[2]) + self.uavVelNED[1] * math.cos(
                -self.uavAngEular[2])
    ...
    def sendMavOffboardCmd(self, type_mask, coordinate_frame, x, y, z, vx, vy, vz, afx, afy, afz, yaw, yaw_rate):
        ...
        if self.isCom or self.isRealFly:
            self.the_connection.mav.set_position_target_local_ned_send(time_boot_ms, self.the_connection.target_system,
                                                                       self.the_connection.target_component,
                                                                       coordinate_frame, type_mask, x, y, z, vx, vy, vz,
                                                                       afx,
    ...
    def sendMavOffboardAPI(self, type_mask=0, coordinate_frame=0, pos=[0, 0, 0], vel=[0, 0, 0], acc=[0, 0, 0], yaw=0,
                           yawrate=0):
        """send offboard command to PX4, the definition of the mavlink message can be found at
        https://mavlink.io/en/messages/common.html#SET_POSITION_TARGET_LOCAL_NED
        """
        ...
        if self.isCom or self.isRealFly:
            ...
            if self.offMode == 0:
                self.the_connection.mav.set_position_target_local_ned_send(int(time_boot_ms),
                                                                           self.the_connection.target_system,
                                                                           self.the_connection.target_component,
                                                                           coordinate_frame, type_mask, pos[0], pos[1],
                                                                           pos[2],
                ...
                self.the_connection.mav.set_position_target_global_int_send(int(time_boot_ms),
                                                                            self.the_connection.target_system,
                                                                            self.the_connection.target_component,
                                                                            coordinate_frame, type_mask, lat_int,
                                                                            lon_int, pos[2],
                                                                            vel[0], vel[1], vel[2], acc[0],
                                                                            acc[1], acc[2], yaw, yawrate)
            elif self.offMode == 2:
                self.the_connection.mav.set_attitude_target_send(int(time_boot_ms), self.the_connection.target_system,
                                                                 ...
                                                                 [yawrate, pos[0], pos[1], pos[2]], vel[0], vel[1],
                                                                 vel[2], yaw)
            ...
            if self.UDPMode > 1.5:
                if self.offMode == 0:
                    buf = self.mav0.set_position_target_local_ned_encode(int(time_boot_ms),
                                                                         self.the_connection.target_system,
                                                                         self.the_connection.target_component,
                                                                         coordinate_frame, type_mask, pos[0], pos[1],
                                                                         pos[2],
                    ...
                    lat_int = int(pos[0] * 1e7)
                    lon_int = int(pos[1] * 1e7)
                    buf = self.mav0.set_position_target_global_int_encode(int(time_boot_ms),
                                                                          self.the_connection.target_system,
                                                                          self.the_connection.target_component,
    ...
    def sendUE4Cmd(self, cmd, windowID=-1):
        ...
        if windowID < 0:
            if self.ip == '127.0.0.1':
                ...
                self.udp_socket.sendto(buf, (
                '224.0.0.10', 20009))  # multicast address, send to all RflySim3Ds on all PC in LAN
        else:
            if self.ip != '127.0.0.1' and self.ip != '255.255.255.255':
                self.udp_socket.sendto(buf, (
                '127.0.0.1', 20010 + windowID))  # ensure this PC can reciver message under specify IP mode
    ...
    # send the position & angle information to RflySim3D to create a new 3D model or update the old model's states
    def sendUE4Pos2Ground(self, copterID=1, vehicleType=3, MotorRPMSMean=0, PosE=[0, 0, 0], AngEuler=[0, 0, 0],
                          windowID=-1):
        ...
        """
        buf = struct.pack("3i7f", 1234567891, copterID, vehicleType, MotorRPMSMean, PosE[0], PosE[1], PosE[2],
    ...
    def sendUE4PosScale(self, copterID=1, vehicleType=3, MotorRPMSMean=0, PosE=[0, 0, 0], AngEuler=[0, 0, 0],
                        Scale=[1, 1, 1], windowID=-1):
        ...
        """
        buf = struct.pack("3i10f", 1234567890, copterID, vehicleType, MotorRPMSMean, PosE[0], PosE[1], PosE[2],
                          AngEuler[0], AngEuler[1], AngEuler[2], Scale[0], Scale[1], Scale[2])
        if windowID < 0:
            if self.ip == '127.0.0.1':
                for i in range(6):
                    self.udp_socket.sendto(buf, (self.ip, 20010 + i))
    ...
    def sendUE4PosScale2Ground(self, copterID=1, vehicleType=3, MotorRPMSMean=0, PosE=[0, 0, 0], AngEuler=[0, 0, 0],
                               Scale=[1, 1, 1], windowID=-1):
        """send the position & angle information to RflySim3D to create a new 3D model or update the old model's states with changing the scale
            checksum =1234567891 is adopted here to tell UE4 to genearete a object always fit the ground
            struct SOut2SimulatorSimple1 {
        ...
        """
        buf = struct.pack("3i10f", 1234567891, copterID, vehicleType, MotorRPMSMean, PosE[0], PosE[1], PosE[2],
                          AngEuler[0], AngEuler[1], AngEuler[2], Scale[0], Scale[1], Scale[2])
        if windowID < 0:
            if self.ip == '127.0.0.1':
                for i in range(6):
    ...
    def sendUE4PosFull(self, copterID, vehicleType, MotorRPMS, VelE, PosE, RateB, AngEuler, windowID=-1):
        ...
        buf0 = struct.pack("2i1d27f3d", copterID, vehicleType, runnedTime, *VelE, *PosE, *AngEuler, *AngQuatern,
                           *MotorRPMS, *AccB, *RateB, *PosGPS)
        # buf for remaining 192-152=40bytes of payload[192] of netDataShort
    ...
    def sendUE4PosNew(self, copterID=1, vehicleType=3, PosE=[0, 0, 0], AngEuler=[0, 0, 0], VelE=[0, 0, 0], PWMs=[0] * 8,
                      runnedTime=-1, windowID=-1):
        """ send the position & angle information to RflySim3D to create a new 3D model or update the old model's states
            # //输出到模拟器的数据
            # struct SOut2SimulatorSimpleTime {
    ...
    def sendUE4PosScale100(self, copterID, vehicleType, PosE, AngEuler, MotorRPMSMean, Scale, isFitGround=False,
                           windowID=-1):
        ...
        if isFitGround:
            checksum = 1234567891
        buf = struct.pack("i200H600f300H100f", checksum, *copterID, *vehicleType, *PosE, *AngEuler, *Scale,
                          *MotorRPMSMean)
        if windowID < 0:
            if self.ip == '127.0.0.1':
                for i in range(6):
    ...
    def sendTakeoffMode(self, alt=0):
        ...
        self.SendSetMode(PX4_CUSTOM_MAIN_MODE.PX4_CUSTOM_MAIN_MODE_AUTO,
                         PX4_CUSTOM_SUB_MODE_AUTO.PX4_CUSTOM_SUB_MODE_AUTO_TAKEOFF)
        if not self.isArmed:
    ...
    def sendMavTakeOff(self, xM=0, yM=0, zM=0, YawRad=0, PitchRad=0):
        ...
        if self.UDPMode > 1.5:
            ...
            lon = lla[1]
            alt = lla[2]
            self.SendMavCmdLong(mavlink2.MAV_CMD_NAV_TAKEOFF, PitchRad / math.pi * 180, 0, 0, YawRad / math.pi * 180,
                                lat, lon, alt)
        else:
            ctrls = [xM, yM, zM, 0]
    ...
    def sendMavSetParam(self, param_id, param_value, param_type):
        ...
        if self.isInPointMode or self.UDPMode < 1.5:
            return
        if self.isCom or self.isRealFly:
            self.the_connection.mav.param_set_send(self.the_connection.target_system,
                                                   self.the_connection.target_component, param_id, param_value,
                                                   param_type)
        else:
            buf = self.mav0.param_set_encode(self.the_connection.target_system, self.the_connection.target_component,
                                             param_id, param_value, param_type).pack(self.mav0)
            self.udp_socket.sendto(buf, (self.ip, self.port))
    ...
    def RcSendLoop(self):
        ...
        while True:
            ...
            # print('RC send')
            self.SendRcOverride(self.RCPWMs[0], self.RCPWMs[1], self.RCPWMs[2], self.RCPWMs[3], self.RCPWMs[4],
                                self.RCPWMs[5], self.RCPWMs[6], self.RCPWMs[7])
            self.sendMavManualCtrl((self.RCPWMs[1] - 1500) * 2, (self.RCPWMs[0] - 1500) * 2, (self.RCPWMs[2] - 1000),
                                   (self.RCPWMs[3] - 1500) * 2)
        ...
        while True:
            ...
            if self.UDPMode > 1.5:  # If Use MAVLink Mode
                msg = self.the_connection.recv_match(
                    type=['ATTITUDE', 'LOCAL_POSITION_NED', 'HEARTBEAT', 'HOME_POSITION', 'GLOBAL_POSITION_INT',
                          'HIL_ACTUATOR_CONTROLS', 'STATUSTEXT', 'ATTITUDE_TARGET', 'BATTERY_STATUS'],
                    blocking=True)
                if msg is not None:
...
class UEMapServe:
    ...
    def getTerrainAltData(self, xin, yin):
        ...
        zz = binmap[intRowInt, intColInt] * (1 - b) * (1 - a) + binmap[intRowInt1, intColInt] * b * (1 - a) + binmap[
            intRowInt, intColInt1] * (1 - b) * a + binmap[intRowInt1, intColInt1] * b * a
        return zz
